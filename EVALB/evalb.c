...
#define MAX_SENT_LEN 110000
#define MAX_WORD_IN_SENT 3000
#define MAX_BRACKET_IN_SENT 500000
...
/*----------------*/
void
    read_parameter_file(filename) char *filename;
{
...
void
    set_param(param, value) char *param,
    *value;
{
...
/*-----------------------------*/
int
    read_line(buff, terminal, quotterm, wn, bracket, bn) char *buff;
s_terminal terminal[];
s_term_ind quotterm[];
int *wn;
s_bracket bracket[];
...
void calc_result(unsigned char *buf1, unsigned char *buf)
{
    ...
    individual_result(wn1, r_bn1, r_bn2, match, bracket_match, crossing, correct_tag);
}
...
/*-----------------------------------------------*/
void
    individual_result(wn1, bn1, bn2, match, bracket_match, crossing, correct_tag) int wn1,
    bn1, bn2, match, bracket_match, crossing, correct_tag;
{
    ...
    else
    {
        ...
        TOTAL_bn2 += bn2;
        TOTAL_match += match;
        TOTAL_bracket_match += bracket_match;
    ...
    if (r_wn1 <= TOT_cut_len)
    {
        ...
        else
        {
            ...
            TOT40_bn2 += bn2;
            TOT40_match += match;
            TOT40_bracket_match += bracket_match;
            if (bn1 == bn2 && bn2 == match)
            {
                TOT40_comp_sent++;
            }
...
void print_total()
{
    ...
    ur = TOTAL_bn1 > 0 ? 100.0 * TOTAL_bracket_match / TOTAL_bn1 : 0.0;
    printf("Unlabeled Bracketing Recall     = %6.2f\n", ur);

    up = TOTAL_bn2 > 0 ? 100.0 * TOTAL_bracket_match / TOTAL_bn2 : 0.0;
    printf("Unlabeled Bracketing Precision  = %6.2f\n", up);

    uf = 2 * up * ur / (up + ur);
    printf("Unlabeled Bracketing FMeasure   = %6.2f\n", uf);
    ...
    ur = TOT40_bn1 > 0 ? 100.0 * TOT40_bracket_match / TOT40_bn1 : 0.0;
    printf("Unlabeled Bracketing Recall     = %6.2f\n", ur);

    up = TOT40_bn2 > 0 ? 100.0 * TOT40_bracket_match / TOT40_bn2 : 0.0;
    printf("Unlabeled Bracketing Precision  = %6.2f\n", up);
...
int
    is_deletelabel_for_length(s) char *s;
{
    int i;
